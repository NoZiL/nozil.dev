...
| `GSC_SA_KEY`      | GitHub Actions repo secret                  | deploy  | Google service-account JSON for the Search Console sitemap submit in the `reindex-google` job (`deploy.yml`; optional, skips if unset — setup in docs/discoverability.md) |
